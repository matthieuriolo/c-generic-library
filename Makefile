...
DIST_COMMON = README $(am__configure_deps) \
	$(nobase_libinclude_HEADERS) $(srcdir)/Makefile.am \
...
ETAGS = etags
CTAGS = ctags
DIST_SUBDIRS = m4 doc src lib tests units
...
manDIR = ./doc/man/man3/
man_MANS = $(manDIR)comparisons.c.3 $(manDIR)BinaryTree.3 \
	$(manDIR)/_array_iter.3 $(manDIR)/_prio_queue.3 \
	$(manDIR)/data_types.h.3 $(manDIR)/list.h.3 \
	$(manDIR)/DequeList.3 $(manDIR)/_array_struct.3 \
	$(manDIR)/_prio_queue_node.3 $(manDIR)/deque.h.3 \
	$(manDIR)/node.h.3 $(manDIR)/List.3 $(manDIR)/_bfs_iter.3 \
	$(manDIR)/_ptr_iter.3 $(manDIR)/error_macros.h.3 \
	$(manDIR)/priority_queue.h.3 $(manDIR)/ListIter.3 \
	$(manDIR)/_closed_hash_iterator.3 $(manDIR)/_ptr_struct.3 \
	$(manDIR)/function_macros.h.3 $(manDIR)/queue.h.3 \
	$(manDIR)/QueueList.3 $(manDIR)/_closed_hash_table.3 \
	$(manDIR)/access_macros.h.3 $(manDIR)/function_signatures.h.3 \
	$(manDIR)/stack.h.3 $(manDIR)/StackList.3 \
	$(manDIR)/_dfs_iter.3 $(manDIR)/binary_tree.h.3 \
	$(manDIR)/gen_macros.h.3 $(manDIR)/stdint.h.3 \
	$(manDIR)/_Hash_List_Node.3 $(manDIR)/_functor_api.3 \
	$(manDIR)/comparisons.h.3 $(manDIR)/hashtable.h.3 \
	$(manDIR)/vector.h.3 $(manDIR)/_Hash_Node.3 \
	$(manDIR)/_open_hash_iterator.3 \
...
install-nobase_libincludeHEADERS: $(nobase_libinclude_HEADERS)
	...
	@list='$(nobase_libinclude_HEADERS)'; test -n "$(libincludedir)" || list=; \
	$(am__nobase_list) | while read dir files; do \
	  ...
	  test -z "$$xfiles" || { \
	    test "x$$dir" = x. || { \
	      echo "$(MKDIR_P) '$(DESTDIR)$(libincludedir)/$$dir'"; \
	      $(MKDIR_P) "$(DESTDIR)$(libincludedir)/$$dir"; }; \
	    echo " $(INSTALL_HEADER) $$xfiles '$(DESTDIR)$(libincludedir)/$$dir'"; \
	    $(INSTALL_HEADER) $$xfiles "$(DESTDIR)$(libincludedir)/$$dir" || exit $$?; }; \
	done

uninstall-nobase_libincludeHEADERS:
	@$(NORMAL_UNINSTALL)
	@list='$(nobase_libinclude_HEADERS)'; test -n "$(libincludedir)" || list=; \
